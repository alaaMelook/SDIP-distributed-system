...
\set auth_pw `cat /run/secrets/db_auth_password 2>/dev/null || echo 'auth_secret'`
\set doc_pw  `cat /run/secrets/db_doc_password 2>/dev/null || echo 'doc_secret'`
\set audit_pw `cat /run/secrets/db_audit_password 2>/dev/null || echo 'audit_secret'`
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'doc_svc') THEN
    CREATE ROLE doc_svc LOGIN PASSWORD 'doc_secret';
  END IF;
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'audit_svc') THEN
    CREATE ROLE audit_svc LOGIN PASSWORD 'audit_secret';
  END IF;
END
$$;
